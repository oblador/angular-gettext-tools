...
var Extractor = (function () {
    function Extractor(options) {
        this.options = _.extend({
            ...
            dustRegex: (/\{@translate( n=[^ }]*)?( plural="([^"]*)")?( n=[^ }]*)?\}(.*?)\{\/translate\}/g),
    ...
    Extractor.isValidStrategy = function (strategy) {
        return strategy === 'html' || strategy === 'js' || strategy === 'dust';
    };
    ...
    Extractor.prototype.extractDust = function (filename, src) {
        var extractDust = function (src, lineNumber) {
            var self = this;
            ...
            var matches;
            while (matches = this.options.dustRegex.exec(src)) {
                this.addString(reference(matches.index), matches[5], matches[3]);
            }
        }.bind(this);
    ...
    Extractor.prototype.parse = function (filename, content) {
        ...
        var strategy = this.options.extensions[extension];
        if (strategy) {
            strategy = 'extract' + strategy.substr(0, 1).toUpperCase() + strategy.substr(1);
            this[strategy](filename, content);
        }
    };
